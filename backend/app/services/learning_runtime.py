...
async def _reduce_event(db: AsyncSession, event: EvidenceEvent):
    ...
    if et == "vnext_personal_path_node_added":
        ...
        nodes[node_id] = {
            ...
            "source_kind": str(node.get("sourceKind") or node.get("source_kind") or "conversation")[:40],
            "source_label": str(node.get("sourceLabel") or node.get("source_label") or "")[:160],
            "source_refs": list(node.get("sourceRefs") or node.get("source_refs") or [])[:8],
            "semantics": list(node.get("semantics") or [])[:12],
