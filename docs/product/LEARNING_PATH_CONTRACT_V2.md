...
Role Atlas 已导入本仓并引用同一份 LearnFlow TypeScript 校验器；`npm run learning-path:sync` 生成与权威导出逐字一致的 v1/v2 静态制品。当前仓库没有把特殊节点接收、原子追加、幂等持久化或跨产品语义挂载伪装成已实现 API；旧 v1 matcher 保持兼容，不能用于自动 `equivalent`。这些服务端接收能力属于后续阶段，运行时必须继续遵守主体、版本和证据边界。
